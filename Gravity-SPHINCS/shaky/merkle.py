...
from finished.wots import WotsSign, WotsSK, LwotsPK, wots_gensk, lwots_genpk, wots_sign
...
@lru_cache  # caching testing utils
...
def merkle_sign_test():
    ...
    for i in range(MERKLE_h):
        auth += (hash_to_bytes(sign.auth[i]).hex())
    wots = ""
    for i in range(WOTS_ell):
        wots += (hash_to_bytes(sign.wots.s[i]).hex())
