...
from LEDAkem.circulant_matrix_helper import circulant_transpose, circulant_matrix_mod
...
def encrypt_niederreiter(M, n0, p, t, sha3, TRNG_byte_len):
    ...
    for i in range(n0 - 1):
        circt_res = circulant_transpose(e[i, :], p)
        circmm_res = circulant_matrix_mod(circt_res, M[i])
        helper_arr = gf2x_add(helper_arr, circmm_res)
